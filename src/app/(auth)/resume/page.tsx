 ...
 *
 * This page is variant-fixed ('professional') — setVariant() is called
 * in the progress-restore useEffect, not via the analytics hook's callback.
 * Defining this outside the component gives it a stable identity across
 * renders, so it never appears as a changed dependency in useOnboardingAnalytics.
 */
...
export default function CareerOnboardingPage() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      redirectingRef.current = true;
      router.replace(user.resume_uploaded ? '/dashboard' : '/resume');
    }
  }, [guardResult, user, router]);
...
function CareerOnboardingContent({
  ...
  //   already called in the progress-restore effect below. The hook requires
  //   the callback for the generic /onboarding page which resolves variant
  //   dynamically. Here it is intentionally a no-op — NOOP_VARIANT_CONFIRMED
  //   is stable (defined outside the component) so it never causes the
  //   useOnboardingAnalytics effect to re-run.
  ...
  useEffect(() => {
    ...
    router.replace(user.resume_uploaded ? '/dashboard' : '/resume');
  }, [user, pendingPostSubmitNav, router]);
