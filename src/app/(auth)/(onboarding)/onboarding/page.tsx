...
export default function OnboardingPage() {
  ...
  // After direction selection, router.push('/onboarding') fires immediately but
  ...
  if (user.user_type === 'student') {
    ...
    return <StudentRedirect router={router} />;
  }
...
function StudentRedirect({ router }: { router: ReturnType<typeof useRouter> }) {
...
function OnboardingContent({
  ...
  const handleChangeDirection = useCallback(async () => {
    try {
      ...
      // does not commit it synchronously. router.replace('/direction') fires
  ...
  }, [resetDirectionMutation, refreshUser, router]);
  ...
  useEffect(() => {
    ...
    if (pendingPostSubmitNav) {
      if (user_type === 'student') {
        ...
        router.replace(resume_uploaded ? '/dashboard' : '/resume');
      }
      return;
    }
  ...
  }, [user, router, pendingPostSubmitNav]);
  ...
  const handleSubmit = useCallback(
    ...
    [variantState, submitOnboarding, refreshUser, router, quota, clearFlowId],
  );
