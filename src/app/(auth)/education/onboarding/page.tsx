 ...
 *   the page called router.replace('/dashboard') immediately. However, the User
...
export default function EducationOnboardingPage() {
  ...
  if (user.student_onboarding_complete || user.onboarding_completed) {
    return <CompletionRedirect router={router} />;
  }
...
function StudentOnboardingContent() {
  ...
  //     1. session.isComplete becomes true → router.replace('/dashboard')
  ...
  //     4. Guard blocks → router.replace('/education/onboarding')
  //     5. Onboarding sees session.isComplete → router.replace('/dashboard')
